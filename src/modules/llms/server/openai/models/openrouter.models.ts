...
import { LLM_IF_HOTFIX_NoTemperature, LLM_IF_OAI_Chat, LLM_IF_OAI_Fn, LLM_IF_OAI_Reasoning, LLM_IF_OAI_Vision } from '~/common/stores/llms/llms.types';
...
export function openRouterModelFamilySortFn(a: { id: string }, b: { id: string }): number {
  ...
  if (aPrefixIndex !== -1 && bPrefixIndex !== -1) return aPrefixIndex !== bPrefixIndex ? aPrefixIndex - bPrefixIndex : b.id.localeCompare(a.id);
...
export function openRouterModelToModelDescription(wireModel: object): ModelDescriptionSchema | null {
  ...
  if (seemsFree) label += ' · 🎁'; // Free? Discounted?
  ...
  const hidden = orOldModelIDs.some((prefix) => model.id.startsWith(prefix)) || !orModelFamilyOrder.some((prefix) => model.id.startsWith(prefix));
  ...
  return fromManualMapping([], model.id, undefined, undefined, {
    ...
    // trainingDataCutoff: ...
    // NOTE: GPT-5 family via OpenRouter should not expose temperature
    // TODO: UI: disable temperature slider when model.interfaces includes `hotfix-no-temperature`
    interfaces: [LLM_IF_OAI_Chat, ...(model.id.startsWith('openai/gpt-5') ? [LLM_IF_HOTFIX_NoTemperature] : [])],
    // benchmark: ...
    chatPrice,
    hidden,
  });
...
export function openRouterInjectVariants(models: ModelDescriptionSchema[], model: ModelDescriptionSchema): ModelDescriptionSchema[] {
  ...
  if (antThinkingModels.includes(model.id)) {
    ...
    const thinkingVariant: ModelDescriptionSchema = {
      ...
      parameterSpecs: [...(model.parameterSpecs || []), { paramId: 'llmVndAntThinkingBudget', initialValue: 1024 }],
    };
