...
export function MCPSettings() {
  ...
  const handleAddServer = React.useCallback(() => {
    ...
    const serverId = preset ? preset.id : `mcp-${Date.now()}`;
  ...
  const handleValidateServer = React.useCallback(
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        ...
        <Stack spacing={2}>
          ...
          {presets && (
            <FormControl>
              ...
              <Select value={selectedPreset} onChange={(_, value) => handlePresetChange(value)} placeholder="Choose a preset...">
              ...
              <FormHelperText>Select a preset configuration or enter custom settings below</FormHelperText>
            </FormControl>
          )}

          <FormControl>
            <FormLabel>Server Name</FormLabel>
            <Input value={newServerName} onChange={(e) => setNewServerName(e.target.value)} placeholder="e.g., Filesystem Tools" />
          </FormControl>
          ...
          <FormControl>
            ...
            />
            <FormHelperText>The command to start the MCP server</FormHelperText>
          </FormControl>

          <Button variant="solid" color="primary" onClick={handleAddServer} disabled={!newServerName || !newServerCommand} startDecorator={<AddIcon />}>
            Add Server
          </Button>
        </Stack>
      </Card>
              ...
              <Card key={server.id} variant="outlined">
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    ...
                    />
                    <IconButton size="sm" color="danger" onClick={() => removeServer(server.id)} disabled={server.enabled}>
                      <DeleteIcon />
                    </IconButton>
                  </Box>
                </Box>
      ...
      <Alert variant="soft" color="primary">
        <Box>
          <Typography level="body-sm">MCP servers extend AI capabilities with tools for file access, web browsing, database queries, and more.</Typography>
          <Typography level="body-sm" sx={{ mt: 1 }}>
            <strong>Quick Test:</strong> Add the &quot;Filesystem&quot; preset above, toggle it ON, then ask the AI to &quot;Create a file at /tmp/hello.txt
            with &apos;Hello MCP!&apos;&quot; or &quot;Read the file at /tmp/hello.txt&quot;
          </Typography>
        </Box>
      </Alert>
    </Box>
