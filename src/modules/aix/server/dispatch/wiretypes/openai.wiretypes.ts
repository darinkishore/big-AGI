...
export namespace OpenAIWire_ContentParts {
  ...
  export const ContentPart_schema = z.discriminatedUnion('type', [TextContentPart_schema, ImageContentPart_schema, OpenAI_AudioContentPart_schema]);
...
export namespace OpenAIWire_Messages {
  ...
  export const AssistantMessage_schema = z.object({
     ...
     */
    tool_calls: z.array(OpenAIWire_ContentParts.ToolCall_schema).optional().nullable(), // [Mistral] added .nullable()
    /**
...
export namespace OpenAIWire_API_Chat_Completions {
  ...
  export const Request_schema = z.object({
    ...
    stream_options: z
      ...
      .optional(),
    reasoning_effort: z.enum(['minimal', 'low', 'medium', 'high']).optional(), // GPT-5 adds 'minimal'
    include_reasoning: z.boolean().optional(), // [OpenRouter, 2025-01-24] enables reasoning tokens
    reasoning: z
      .object({
        // [OpenRouter, 2025-06-05] Reasoning parameter for Claude models
        max_tokens: z.number().int().positive(),
      })
      .optional(),
    prediction: z
      .object({
        // [OpenAI, 2024-11-05] Predicted Outputs - for regenerating a file with only minor changes to most of the content.
    ...
    // GPT-5: control verbosity at request root (Chat Completions)
  ...
  const Choice_NS_schema = z.object({
    ...
    finish_reason: z.union([FinishReason_Enum, z.string()]).nullable(),
  ...
  export const Response_schema = z.object({
    ...
    system_fingerprint: z
      .string()
      .optional() // The backend configuration that the model runs with.
  ...
  const _UndocumentedError_schema = z.object({
    ...
    code: z.string().nullable().optional().or(z.number()), // [OpenRouter, 2024-11-21] code can be a number too
  ...
  const ChunkDeltaToolCalls_schema = z.object({
    index: z
      .number() // index is not present in non-streaming calls
  ...
  const ChunkDelta_schema = z.object({
    role: z.literal('assistant').optional().nullable(), // [Deepseek] added .nullable()
    // delta-text content
    content: z.string().nullable().optional(),
    ...
    reasoning: z
      .string()
      .optional() // [OpenRouter, 2025-01-24]
      .nullable(), // [OpenRouter, 2025-06-05] null on Anthropic text responses past the reasoning blocks
    // delta-tool-calls content
    tool_calls: z.array(ChunkDeltaToolCalls_schema).optional().nullable(), // [TogetherAI] added .nullable(), see https://github.com/togethercomputer/together-python/issues/160
  ...
  const ChunkChoice_schema = z.object({
    index: z.number().optional(), // [OpenRouter] added .optional() which implies index=0 I guess

    // A chat completion delta generated by streamed model responses.
    delta: ChunkDelta_schema,

    finish_reason: z
      .union([FinishReason_Enum, z.string()])
  ...
  export const ChunkResponse_schema = z.object({
    ...
    usage: Usage_schema.optional(), // If requested
    created: z
      .number() // The Unix timestamp (in seconds) of when the chat completion was created.
  ...
  const Request_schema = z.object({
    ...
    quality: z
      .enum([
        ...
        'low', // gpt-image-1
        'hd',
        'standard', // dall-e-3: hd | standard, dall-e-2: only standard
      ])
      .optional(),
...
export namespace OpenAIWire_Responses_Items {
  ...
  export const _ContentItem_Parts_schema = z.union([ContentItem_TextPart_schema, ContentItem_RefusalPart_schema]);
  ...
  const UserItemMessage_schema = _InputItem_schema.extend({
    ...
    content: z.array(z.union([Input_TextPart_schema, Input_ImagePart_schema, Input_FilePart_schema])),
  });
  ...
  const InputMessage_Compat_schema = z.union([_InputMessage_Compat_User_schema, _InputMessage_Compat_Model_schema]);
...
export namespace OpenAIWire_Responses_Tools {
  ...
  export const Tool_schema = z.union([
    ...
    // custom text tools (GPT-5) - removed for now
    // hosted tools
...
export namespace OpenAIWire_API_Responses {
  ...
  export const Request_schema = z.object({
    ...
    reasoning: z
      .object({
        effort: z.enum(['minimal', 'low', 'medium', 'high']).nullish(), // defaults to 'medium'; GPT-5 adds 'minimal'
    ...
    text: z
      .object({
        format: z
          ...
          .optional(),
        // GPT-5: control verbosity of text outputs
